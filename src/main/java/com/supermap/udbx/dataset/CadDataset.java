 ...
 * {@link CadFeature#attributes()} 中。
 */
public class CadDataset extends VectorDataset {

    /** 数据表中的几何列名（GeoHeader 格式 BLOB）。 */
     ...
     */
    public List<CadFeature> getFeatures() {
     ...
     * @throws RuntimeException 若数据库查询失败
     */
    public CadFeature getFeature(int smId) {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            throw new RuntimeException("读取 CAD 要素 SmID=" + smId + " 失败", e);
        }
    }
     ...
     * @param geometry   CAD 几何对象（将被序列化为 GeoHeader 格式 BLOB）
     * @param attributes 用户属性字段（暂不写入，预留接口）
     * @throws RuntimeException 若写入失败
     */
    public void addFeature(int smId, CadGeometry geometry, Map<String, Object> attributes) {
        byte[] geomBytes = CadGeometryWriter.write(geometry);
        ...
        try {
            ...
            throw new RuntimeException("写入 CAD 要素 SmID=" + smId + " 失败", e);
        } finally {
    ...
    public void deleteFeature(int smId) {
        ...
        try {
            ...
            try { conn.rollback(); } catch (SQLException ignored) {}
            throw new RuntimeException("删除 CAD 要素 SmID=" + smId + " 失败: " + e.getMessage(), e);
        } finally {
            try { conn.setAutoCommit(true); } catch (SQLException ignored) {}
        }
    }

    public void updateFeature(int smId, com.supermap.udbx.geometry.cad.CadGeometry geometry,
        ...
        try {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sqlBuf.toString())) {
                int idx = 1;
                if (geometry != null) stmt.setBytes(idx++, com.supermap.udbx.geometry.cad.CadGeometryWriter.write(geometry));
            ...
            try { conn.rollback(); } catch (SQLException ignored) {}
            throw new RuntimeException("更新 CAD 要素 SmID=" + smId + " 失败: " + e.getMessage(), e);
        } finally {
            try { conn.setAutoCommit(true); } catch (SQLException ignored) {}
        }
    }
    ...
    private CadFeature mapRow(ResultSet rs, List<String> userColumns) throws SQLException {
        ...
        return new CadFeature(smId, geometry, attributes);
    }
}
